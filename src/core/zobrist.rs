...
impl Zobrist {
    ...
    pub fn transformed_table() -> &'static [[[u64; 8]; 2]; 225] {
        static TRANS_TABLE: OnceLock<[[[u64; 8]; 2]; 225]> = OnceLock::new();
        TRANS_TABLE.get_or_init(|| {
            let base_table = Self::table();
            let mut t_table = [[[0u64; 8]; 2]; 225];
            for y in 0..15 {
                for x in 0..15 {
                    let idx = y * 15 + x;
                    for t in 0..8 {
                        let t_idx = Self::get_transformed_idx(x as i8, y as i8, t);
                        t_table[idx][0][t] = base_table[t_idx][0];
                        t_table[idx][1][t] = base_table[t_idx][1];
                    }
                }
            }
            t_table
